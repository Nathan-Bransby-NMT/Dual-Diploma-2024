...
list.ActionBarController.prototype =
{
    ...
    onActionItemClick: function( event, minSelected, onClick, errorMsg )
    {
      ...
      if( thisCheckBoxes.length !== 0 )
      {
        ...
        {
          var listContainer = event.target.closest('li.sub');
          var threadActionButton = listContainer.querySelector('.listContainer')

          bb_dialogs.bb_confirm({
            title: page.bundle.getString("dialog.alert"),
            html: '<div role="alertdialog" aria-modal="true" aria-live="assertive" aria-describedby="alert-message" aria-atomic="true">'+
                  '<p id="alert-message" aria-live="assertive" >' + errorMsg + '</p>' +
                  '</div>',
            onClose: function () {
              threadActionButton.focus();
            },
            buttons: [
              {
                text: page.bundle.getString("closeStr"),
              },
            ],
        });
          Event.stop( event );
